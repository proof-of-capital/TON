...
- [Основные положения](#основные-положения)
  - [Переменные, задаваемые при инициализации и деплое контракта](#переменные-задаваемые-при-инициализации-и-диплое-контракта)
...
- [Обращение к гетер-функциям](#обращение-к-гетер-функциям)
...
### Переменные, задаваемые при инициализации и диплое контракта

При инициализации и диплое контракта в Tact задаются следующие переменные, которые определяют основные настройки контракта и его начальное состояние:
...
2. **Взаимодействие с контрактом:**
   - **Market Maker**:
     - Во время действия лока обмениваться токенами с контрактом может только маркет-мейкер.
   ...
   - **Другие пользователи**:
     ...
     - Они могут вернуть приобретенные у контракта жетоны обратно в контракт в обмен на обеспечение.
   - **Return Wallet**:
...
3. **Завершение лока:**
   - После окончания лока создатель может вывести все монеты и все TON с контракта.
   
### Важные замечания

- **Используйте гетер-функции:**
  - При работе с контрактом обязательно используйте гетер-функции для проверки результатов своих операций **до** их выполнения.
...
- **Взаимодействие с контрактом:**
  ...
  - Если вы отправите TON напрямую:
    - Контракт может отклонить операцию, если до окончания лока осталось более двух месяцев.
    - Контракт может отправить вам монеты (жетоны), если до окончания лока осталось меньше двух месяцев.
    - Обязательно проверьте результаты гетер-функций перед взаимодействием с контрактом. Если у контракта не достаточно жетонов, то он отправит все, что осталось. Если монет не осталось, то TON просто перейдут на баланс контракта.
    

- **Особенности продажи остатка жетонов:**
  - Если вы попытаетесь вернуть большее количество жетонов, чем контракт должен выкупить, то выплата составит весь оставшийся баланс TON, зарезервированный на выкуп монет, а излишки монет перейдут на баланс контракта. 
  - Обязательно перед проведением процедуры отправки жетонов на контракт изучите вывод гетер-функции `jetton_available`.
 
- **Прибыль и комиссии:**
  - **20%** от прибыли направляется на кошелёк **Royalty**, указанный на сайте [proofofcapital.org](https://proofofcapital.org).
  - **80%** направляются на кошелек создателя в качестве фонда на разработку и маркетинг.

---

## Обращение к гетер-функциям

Для получения данных о текущем состоянии контракта и результатов внутренних функций вы можете использовать гетер-функции:

1. Перейдите в **TON Viewer** или аналогичный инструмент.
2. Откройте вкладку **Methods**.
3. В поле **Arbitrary method** введите название гетер-функции.
4. При необходимости добавьте аргументы.
5. Нажмите кнопку **Execute** для выполнения запроса.

**Рекомендуем** всегда обращаться к гетер-функциям перед выполнением операций для проверки ожидаемых результатов.
...
**Внимание!** Отправка монет напрямую на контракт и проведение транзакций без предварительного анализа вывода гетер-функций могут привести к потере средств. Всегда проверяйте адрес и используйте предусмотренные методы взаимодействия с контрактом.