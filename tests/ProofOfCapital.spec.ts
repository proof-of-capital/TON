...
describe('ProofOfCapital', () => {
    ...
    // TODO: Add sanity checks for calculate-functions (e.g. they never return negative value).
    //       A good idea to test it might be implemented property-based tests with random
    //       inputs using e.g. the fast-check framework.
    ...
    it("get ton_to_jetton_calculation works", async () => {
      const takeWalletAddressResult = await proofOfCapital.send(
      ...
      expect(takeWalletAddressResult.transactions).toHaveTransaction({
      ...
      const availableJettons = 10000000000n;
      ...
      expect(await proofOfCapital.getTonToJettonCalculation(10000000n)).toEqual(availableJettons)
